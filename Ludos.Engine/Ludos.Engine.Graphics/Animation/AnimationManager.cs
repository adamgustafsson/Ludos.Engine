namespace Ludos.Engine.Graphics
{
    ...
    public class AnimationManager
    {
        ...
        {
            var visibleActorAnimations = _actorAnimations.Where(x => _camera.IsOnScreen(x.Value.Actor.Bounds) && x.Value.FrameCount > 1 && x.Key == x.Value.Actor.CurrentState && x.Value.Actor.IsActive).Select(x => x.Value);
            var inactiveActorAnimations = _actorAnimations.Where(x => !_camera.IsOnScreen(x.Value.Actor.Bounds) || (x.Key != x.Value.Actor.CurrentState && x.Value.IsAnimating)).Select(x => x.Value);
            ...
            {
                animationsToUpdate.AddRange(_animations.Where(x => _camera.IsOnScreen(x.Bounds) && x.FrameCount > 1));
                animationsToReset.AddRange(_animations.Where(x => !_camera.IsOnScreen(x.Bounds) && x.FrameCount > 1));
            }
            ...
            foreach (var animation in animationsToUpdate)
            {
                ...
                if (animation.Timer > animation.FrameSpeed)
                {
                    ...
                    if (animation.CurrentXFrame >= (animation.StartFrame.X + animation.FrameCount))
                    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            foreach (var animation in visibleAnimations)
            {
                spriteBatch.Draw(
                    ...
                    new Rectangle(
                        animation.CurrentXFrame * animation.FrameWidth,
